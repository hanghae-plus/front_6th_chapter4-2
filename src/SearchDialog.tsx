...
const SearchDialog = ({ searchInfo, onClose }: Props) => {
  ...
  const MajorTag = memo(({ major }: { major: string }) => {
    return (
  ...
  const TimeTag = memo(({ time }: { time: number }) => {
    return (
      ...
      >
        <Wrap spacing={1} mb={2}>
          {searchOptions.majors.map(major => (
            <MajorTag major={major} key={major} />
          ))}
        </Wrap>
        <Stack
          spacing={2}
