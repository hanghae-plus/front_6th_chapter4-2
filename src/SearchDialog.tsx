...
import { getCacheApi } from "./libs/getCacheApi.ts";
...
const fetchMajors = () =>
  getCacheApi({
    key: "fetchMajors",
    queryFn: () => axios.get<Lecture[]>("/schedules-majors.json"),
  });

const fetchLiberalArts = () =>
  getCacheApi({
    key: "fetchLiberalArts",
    queryFn: () => axios.get<Lecture[]>("/schedules-liberal-arts.json"),
  });
// 581
...
const SearchDialog = ({ searchInfo, onClose }: Props) => {
  ...
  const filteredLectures = React.useMemo(() => {
    return getFilteredLectures(searchOptions, lectures);
  }, [searchOptions, lectures]);

  const visibleLectures = React.useMemo(() => {
  ...
  const changeSearchOption = React.useCallback(
