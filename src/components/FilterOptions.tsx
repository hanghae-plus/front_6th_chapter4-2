...
// 필터 옵션 컴포넌트
const FilterOptions = memo(({ searchOptions, allMajors, onChange }: Props) => {
  return (
    <>
      <HStack spacing={4}>
        <FormControl>
            ...
            <Wrap spacing={1} mb={2}>
              {searchOptions.times.sort((a, b) => a - b).map(time => (
                <Tag key={time} size="sm" variant="outline" colorScheme="blue">
                  ...
                  <TagCloseButton
                    onClick={() => onChange('times', searchOptions.times.filter(v => v !== time))}/>
                </Tag>
              ))}
            </Wrap>
            ...
            <Wrap spacing={1} mb={2}>
              {searchOptions.majors.map(major => (
                <Tag key={major} size="sm" variant="outline" colorScheme="blue">
                  ...
                  <TagCloseButton
                    onClick={() => onChange('majors', searchOptions.majors.filter(v => v !== major))}/>
                </Tag>
              ))}
            </Wrap>
            <Stack spacing={2} overflowY="auto" h="100px" border="1px solid" borderColor="gray.200"
